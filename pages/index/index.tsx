...
import { ALL, PROJECT, TEAM } from '../../common/value';
...
const Main: NextPage<Props> = ({ user, firstNoticesSet, token }) => {
	...
	const fetchMoreNotices = async () => {
		if (notices.next !== null)
			try {
				const { data } = await apis.fetchNotices(page + 1, token, type);
	...
	useEffect(() => {
		const fetchNewNotices = async () => {
			try {
				const { data } = await apis.fetchNotices(1, token, type);
				setNotices({
					...data,
		...
		<StyledMain>
			...
			<StyledHead>
				<StyledNav>
					...
					<StyledNavText white={type === ALL} onClick={() => setType(ALL)}>
						전체회의 공지
					</StyledNavText>
					<StyledNavText white={type === TEAM} onClick={() => setType(TEAM)}>
						팀별 스터디 공지
					</StyledNavText>
					<StyledNavText white={type === PROJECT} onClick={() => setType(PROJECT)}>
						앱프로젝트 공지
					</StyledNavText>
				</StyledNav>
				<Link to="/">
			...
			<StyledCardWrapper>
				{notices.results.map((notice: any) => {
					return <Card key={notice.pk} notice={notice} onClose={onClose(notice.pk)} user={user} />;
				})}
			</StyledCardWrapper>
...
Main.getInitialProps = async ({ res, token }: any) => {
	...
	try {
		const { data } = await apis.fetchNotices(1, token, ALL);
		return {
			token,
			user,
